...
func main() {
	...
	peerConnection.OnDataChannel(func(d *webrtc.DataChannel) {
		...
		d.OnOpen(func() {
			fmt.Printf("Proxy: Data channel '%s'-'%d' open. Random messages will now be sent to any connected DataChannels every 5 seconds\n", d.Label(), d.ID())

			go func() {
				time.Sleep(10 * time.Second)
				switchMsg := "switch"
				fmt.Printf("Proxy1: Sending '%s'\n", switchMsg)
				sendTextErr := d.SendText(switchMsg)
				if sendTextErr != nil {
					panic(sendTextErr)
				}
			}()
		...
		d.OnMessage(func(msg webrtc.DataChannelMessage) {
			fmt.Printf("Proxy1: Message from DataChannel '%s': '%s'\n", d.Label(), string(msg.Data))
			if string(msg.Data) == "received switch" {
				stop = true
				jsonBody := []byte(`{"proxy1_message": "this part stimulates the information that needs to be sent to the other proxy"}`)
				bodyReader := bytes.NewReader(jsonBody)
				_, err := http.Post(fmt.Sprintf("http://%s/migration", *answerAddr2), "application/json; charset=utf-8", bodyReader)
				if err != nil {
					panic(err)
				}
				fmt.Printf("Proxy1: Sending '%s'\n", "stop")
				sendTextErr := d.SendText("stop")
				if sendTextErr != nil {
					panic(sendTextErr)
				}
				//d.Close()
	...
	go func() { panic(http.ListenAndServe(*answerAddr1, nil)) }()
